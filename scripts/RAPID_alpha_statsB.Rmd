...
alphadiv <- subset(alphadiv, SampleID != "July2016.228.2" & SampleID != "July2016.251.2" & SampleID != "July2016.251.1")
str(alphadiv)
```
...
emmeans(lmerR, list(pairwise ~ species), adjust = "tukey")
...
#aov(sqrt(richness) ~ time*species*treatment + Error(indiv), data = alphadiv)
# TukeyHSD(aov(sqrt(richness) ~ treatment, data = alphadiv))
#TukeyHSD(aov(sqrt(richness) ~ time, data = alphadiv))
#TukeyHSD(aov(sqrt(richness) ~ species, data = alphadiv)) # doesn't work bc of unbalanced design
```
...
### For Acropora only and removing July data
Subsetting the data to only include Acropora corals
```{r}
alphadiv_acr <- alphadiv[which(alphadiv$species =="ACR"),]
alphadiv_acr_noj <- subset(alphadiv_acr, time != "T3")
summary(alphadiv_acr_noj)
alphadiv <- alphadiv_acr_noj
dim(alphadiv)
```
#### Chao1 data (richness)
Proceeding with sqrt-transformed richness data
```{r rich2}
lmerR <- lmer(sqrt(richness)~ time*treatment + (1|indiv), data = alphadiv)
...
# post hoc tests
emmeans(lmerR, list(pairwise ~ time), adjust = "tukey")
emmeans(lmerR, list(pairwise ~ treatment), adjust = "tukey")
# Old ANOVAs
...
```{r even2}
lmerR <- lmer(asinTransform(evenness)~ time*treatment + (1|indiv), data = alphadiv)
...
Proceeding with sqrt-transformed faithPD data
```{r faithPD2}
lmerR <- lmer(sqrt(faithPD)~ time*treatment + (1|indiv), data = alphadiv)
