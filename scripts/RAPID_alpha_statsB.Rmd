...
TukeyHSD(aov(asinTransform(richness) ~ time, data = alphadiv))
```
...
# TukeyHSD(aov(asinTransform(evenness) ~ treatment, data = alphadiv))
```
#### Faith's phylogenetic diversity data (faithPD)
Proceeding with sqrt-transformed faithPD data
```{r faithPD2}
...
summary(aov(asinTransform(evenness) ~ time*treatment, data = alphadiv))
# This interaction test with Kruskal wallis is not comparable to anova
